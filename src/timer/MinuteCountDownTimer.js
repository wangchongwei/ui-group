...
/** 获取初始化时的秒钟 */
function getInitSecond(duration) {
...
export default class CountDownTimer extends React.PureComponent<Props, State> {
  ...
  state = {
    date: transSecondToMins(getInitSecond(this.props.duration)),
  }
  componentWillMount() {
    this.passTime();
    this._initParams();
  ...
  /** 初始化数据 */
  _initParams = () => {
    const { duration, expireTimeString } = this.props;
    let allSecond = 0;
    if(expireTimeString) {
      // 如果截止日期存在
      const date = CommonFunc.getDateForStr(expireTimeString, true);
      // 总时长等于 截止日期 减去 开始日期
      allSecond = (date.getTime() - this.initTime) / 1000;
    } else {
      // 如果截止日期不存在，则用时长来算出总时长
      allSecond = duration * 60;
    }
